...
function validate_booking_equip($lab_first_name, $lab_last_name, $lab_age, $lab_purpose, $lab_waiver, $lab_card, $lab_phone, $lab_email, $lab_orientation, $lab_time, $lab_date, $lab_name, $lab_branch){
  ...
  if($lab_branch == 7){
    /* this isn't needed anymore due to available staffing from Sept 18, 2017
    $valid_lab_days = array("Tuesday", "Thursday");
    $day_of_week = new Datetime($lab_date);
    $day_of_week = $day_of_week->format("l");

    $valid_saturdays = array();
    $start_saturday = new Datetime("2017-02-25");

    for($i = 0; $i < 26; $i++){ 
      $days_ahead = 14;
      $start_saturday->modify("+$days_ahead days");
      $valid_saturdays[] = $start_saturday->format("Y-m-d");
    }

    //either on the tuesday, thursday, or a valid saturday
    if(!in_array($day_of_week, $valid_lab_days)){
      if(!in_array($lab_date, $valid_saturdays)){
        $str_message .= "Error: The lab is not available for the date requested.\n";
      }      
    }*/    
  }
...
if(isset($_POST['submit_equip'])){
    //prep mail 
    $first_name = filter_var($_POST['lab_first_name_equip'], FILTER_SANITIZE_STRING);
    $branch = filter_var($_POST['branch_equip'], FILTER_SANITIZE_STRING);
    $lab_name = filter_var($_POST['lab_name_equip'], FILTER_SANITIZE_STRING);
    $lab_date = filter_var($_POST['lab_date_equip'], FILTER_SANITIZE_STRING);
    $lab_time_slot = filter_var($_POST['lab_time_equip'], FILTER_SANITIZE_STRING);
    $last_name = filter_var($_POST['lab_last_name_equip'], FILTER_SANITIZE_STRING);
    $lab_age = filter_var($_POST['lab_age_equip'], FILTER_SANITIZE_STRING);
    $lab_agreement = filter_var($_POST['lab_agreement_equip'], FILTER_SANITIZE_STRING);
    $lab_purpose = filter_var($_POST['lab_purpose_equip'], FILTER_SANITIZE_STRING);
    $lab_library_card = filter_var($_POST['lab_library_card_equip'], FILTER_SANITIZE_STRING);
    $lab_email = filter_var($_POST['lab_email_equip'], FILTER_SANITIZE_EMAIL);
    $lab_telephone = filter_var($_POST['lab_telephone_equip'], FILTER_SANITIZE_STRING);
    $lab_orientation = "no";
    ...
    $results = validate_booking_equip($first_name, $last_name, $lab_age, $lab_purpose, $lab_agreement, $lab_library_card, $lab_telephone, $lab_email, $lab_orientation, $lab_time_slot, $lab_date, $lab_name, $branch);
    if(strlen($results) == 0){
...
<article id="post-<?php the_ID(); ?>" <?php post_class('blog-post'); ?>>
  <div class="entry-content" id="form-entry-div">
    ...
    <form action="/shareit/bookings<?php if(!empty($choice)) echo '?choice=' . $choice; ?>" method="post" enctype="multipart/form-data" onsubmit="return validateBooking();">			
          ...
          <p> 
            <select name="branch_equip" title="Local Branch" id="branch_equip" onchange="changeLabs()" required>              
          ...
          <p> 
            <select name="lab_name_equip" id="lab_name_equip" onchange="limitTime();">
          ...
          <p> 
           <input name="lab_date_equip" size="15" maxlength="15" onblur="limitTime()" class="input_space" id="lab_date_equip"  type="text" required>
          </p>
          ...
          <p> 
            <input type="text" name="lab_first_name_equip" id="lab_first_name_equip" class="input_space" size="50" required />
          </p>

          <p> <label for="lab_last_name_equip" class="label_text">Last Name<sup>*</sup>:&nbsp;</label></p>
          <p> 
           <input type="text" name="lab_last_name_equip" class="input_space" id="lab_last_name_equip" size="50" required />
          </p>

          <p> <label for="lab_age_equip" class="label_text">Age<sup>*</sup>:&nbsp;</label></p>
          <p> 
            <input type="text" name="lab_age_equip" class="input_space" id="lab_age_equip" size="50" required />
          </p>

          <p> <label for="lab_agreement_equip" class="label_text">Customer Agreement Form Signed?<sup>*</sup>:&nbsp;</label></p>
          <p> 
            <input type="radio" id="lab_agreement_equip" value="Yes" name="lab_agreement_equip">&nbsp;Yes
            <input type="radio" id="lab_agreement_equip" value="No" name="lab_agreement_equip" checked>&nbsp;No        
          </p>
          ...
          <p> 
           <input type="text" name="lab_library_card_equip" class="input_space" id="lab_library_card_equip" required />
          </p>
     
          <p> <label for="lab_email_equip" class="label_text">Email<sup>*</sup>:&nbsp;</label></p>
          <p> 
            <input type="email" name="lab_email_equip" class="input_space" id="lab_email_equip" size="50" required />
          </p>

          <p> <label for="lab_telephone_equip" class="label_text">Telephone<sup>*</sup>:&nbsp;</label></p>
          <p> 
            <input type="text" name="lab_telephone_equip" class="input_space" id="lab_telephone_equip" required />
          </p>
          ...
          <p class="text_center">
            <input type="submit" name="submit_equip" id="submit_equip" value="Submit Request">
          </p>											              
      </fieldset>
    </form>
...
function limitTime(){
  ...
  /*
  //Tuesday Equipment Times PBRL 
	var pbrl_tuesday_times = [
		{value: "10am", text: "10:00am - 12:00pm"},
		{value: "12pm", text: "12:00pm - 2:00pm"},
		{value: "2pm", text: "2:00pm - 4:00pm"},
	];

  //Thursday Equipment Times PBRL 
	var pbrl_thursday_times = [
		{value: "2pm", text: "2:00pm - 4:00pm"},
		{value: "4pm", text: "4:00pm - 6:00pm"},
		{value: "6pm", text: "6:00pm - 8:00pm"},
	];

  //Saturday Equipment Times PBRL 
	var pbrl_saturday_times = [
		{value: "10am", text: "10:00am - 12:00pm"},
		{value: "12pm", text: "12:00pm - 2:00pm"},
		{value: "2pm", text: "2:00pm - 4:00pm"},
	];*/
...
window.onload = function() {
    ...
    if (window.location.search.indexOf('choice=') > -1) {
      ...
      document.getElementById('lab_date_equip').focus();      
